...
import { getActiveDoctors, getAvailableTimeSlots, createOnlineBooking } from '../lib/supabase';
...
export default function PublicBooking() {
  ...
  const loadTimeSlots = async () => {
    try {
      setLoading(true);
      const slots = await getAvailableTimeSlots(selectedDoctor.id, selectedDate);
      setTimeSlots(slots || []);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createOnlineBooking(bookingData);
            ...
            <div>
              ...
              {selectedDate && (
                <div className="mb-6">
                  ...
                  {timeSlots.filter(slot => slot.is_available).length === 0 && !loading && (
            ...
            <div>
              ...
              <form onSubmit={(e) => { e.preventDefault(); setStep(3); }} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1"><Mail className="inline mr-1" size={14} /> Email *</label>
                    <input type="email" required value={patientData.email} onChange={(e) => setPatientData({...patientData, email: e.target.value})} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
          ...
          {step === 3 && (
            <div>
              ...
              <div className="flex space-x-4">
                ...
                <button onClick={handleSubmit} disabled={loading} className="flex-1 bg-green-600 text-white py-3 rounded-lg font-semibold hover:bg-green-700 transition disabled:opacity-50">{loading ? 'Submitting...' : 'Confirm Booking'}</button>
              </div>
            </div>
          )}
        </div>
...
$content | Out-File -FilePath "src\pages\PublicBooking.jsx" -Encoding UTF8 -NoNewline
Write-Host "PublicBooking.jsx created successfully!"

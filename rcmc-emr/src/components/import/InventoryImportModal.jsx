/**
 * Inventory Import Modal Component
 * 
 * 3-Step wizard for importing inventory items from CSV:
 * Step 1: Upload CSV file
 * Step 2: Preview & Validate data with categorization
 * Step 3: Import Progress & Results
 * 
 * Requirements: 3.1, 9.5, 12.8
 */

import { useState, useEffect } from 'react';
import { parseCSV } from '../../utils/import/csvParser';
import { validateInventoryData, batchImportInventory, previewCategorization } from '../../services/import/inventoryImportService';
 ...
 * @param {Function} props.onSuccess - Success callback (refresh inventory list)
 */
export function InventoryImportModal({ isOpen, onClose, onSuccess }) {
  ...
  async function validateAndCheckDuplicates() {
    ...
    try {
      // Validate data
      const errors = validateInventoryData(parsedData.data);
      setValidationErrors(errors);

      // Preview categorization
      const preview = previewCategorization(parsedData.data);
  ...
  async function startImport() {
    ...
    try {
      ...
      const importResult = await batchImportInventory(rowsToImport, (progressUpdate) => {
        setProgress({
  ...
  function handleDownloadValidationErrors() {
    exportValidationErrors(validationErrors, 'inventory-import-errors.csv');
  }

  function handleDownloadResults() {
    exportResults(result, 'inventory-import-results.csv');
  }

  function handleDownloadErrors() {
    if (result && result.errors) {
      exportErrors(result.errors, 'inventory-import-failed-records.csv');
    }
  }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-900">Import Inventory Items</h2>
            <button
              onClick={handleClose}
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          Upload a CSV file containing inventory items (medicines, supplies, and services).
        </p>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="text-xs text-blue-800 space-y-1">
          <p><strong>Required columns:</strong> Item Name, Price</p>
          <p><strong>Optional columns:</strong> Unit, Stock</p>
          <p><strong>Automatic categorization:</strong> Items will be categorized as Services, Medicines, or Medical Supplies</p>
          <p><strong>Price format:</strong> Numeric value (e.g., 150.00)</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Review the data, categorization, and resolve any validation errors before importing.
        </p>
      </div>

      {isLoading && (
        <div className="flex justify-center py-8">
          <LoadingSpinner message="Validating data and categorizing items..." />
        </div>
      )}
        ...
        <>
          ...
          {categorizationPreview && (
            <div className="border border-blue-200 rounded-lg p-4 bg-blue-50">
              <h4 className="text-sm font-semibold text-blue-900 mb-3">Automatic Categorization Preview</h4>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Services</p>
                  <p className="text-2xl font-bold text-blue-600">{categorizationPreview.breakdown.Services}</p>
                </div>
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Medicines</p>
                  <p className="text-2xl font-bold text-green-600">{categorizationPreview.breakdown.Medicines}</p>
                </div>
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Medical Supplies</p>
                  <p className="text-2xl font-bold text-purple-600">{categorizationPreview.breakdown.Medical_Supplies}</p>
                </div>
              </div>
              <p className="text-xs text-blue-800 mt-3">
                Items are automatically categorized based on keywords and patterns in the item names.
              </p>
            </div>
          )}
              ...
              <div className="max-h-40 overflow-y-auto space-y-2">
                {duplicates.slice(0, 5).map((dup, index) => (
                  <div key={index} className="text-xs text-yellow-800 bg-white p-2 rounded">
                    Duplicate: {dup.importRow.item_name || dup.importRow['Item Name'] || dup.importRow.name}
                  </div>
                ))}
              </div>
            </div>
      ...
      {result && !isLoading && (
        <>
          ...
          {result.categoryBreakdown && (
            <div className="border border-blue-200 rounded-lg p-4 bg-blue-50">
              <h4 className="text-sm font-semibold text-blue-900 mb-3">Category Breakdown</h4>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Services</p>
                  <p className="text-2xl font-bold text-blue-600">{result.categoryBreakdown.Services || 0}</p>
                </div>
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Medicines</p>
                  <p className="text-2xl font-bold text-green-600">{result.categoryBreakdown.Medicines || 0}</p>
                </div>
                <div className="bg-white rounded p-3">
                  <p className="text-xs text-gray-600 mb-1">Medical Supplies</p>
                  <p className="text-2xl font-bold text-purple-600">{result.categoryBreakdown.Medical_Supplies || 0}</p>
                </div>
              </div>
            </div>
          )}

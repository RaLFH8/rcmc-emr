const StatCard = ({ title, value, trend, icon: Icon, iconBg }) => {
  ...
  // Extract color from iconBg gradient for the icon color
  const getIconColor = () => {
    if (iconBg.includes('teal')) return 'text-teal-500'
    if (iconBg.includes('purple')) return 'text-purple-500'
    if (iconBg.includes('pink')) return 'text-pink-500'
    ...
    <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-100">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-xl bg-slate-50 flex items-center justify-center">
          <Icon className={`w-6 h-6 ${getIconColor()}`} strokeWidth={1.5} />
        </div>
